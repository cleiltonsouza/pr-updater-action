...
export async function buildDescription() {
  ...
  const currentDescription = await getCurrentPullRequestDescription(repoOwner, repoName, prNumber)
  const commitMessages: string[] = await getCommitMessages(repoOwner, repoName, prNumber);
